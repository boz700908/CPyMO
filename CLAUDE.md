...
- **Unified input**: Native gesture adapters and SDL2 copy shortcuts enqueue `cpymo_accessibility_action` values through `cpymo_backend_input.h`; `cpymo_input_snapshot()` is the only consumer. Pending actions are merged atomically so a hold-end action cannot be overwritten by a following gesture. Copy and append-copy must be edge-triggered, never inferred from a continuously polled key or controller state. Do not synthesize keyboard events, mutate engine state, or maintain a platform-local copy cache in a gesture bridge.
