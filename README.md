...
在 GitHub Actions 手动运行`CI`时，可选填`web_game_archive_url`（游戏 ZIP 压缩包直链）及`web_build_type`（`js`或`wasm`）。工作流支持重定向和失败重试；两项同时提供时会下载、解压并上传独立的`CPyMO Web Game`附件，并固定使用 FFmpeg 音视频后端以保留游戏视频支持。
...
- 桌面端 SDL2 输出原始 PCM 时会对 WAV 做 2 倍增益（+6dB），以匹配 Android SoundPool 在系统音频通路中的感知音量
...
**复制朗读内容**：可将上次朗读的文字复制到剪贴板，与 Android 无障碍手势对齐：
- **键盘**：Shift+C 复制，Shift+D 追加复制（换行分隔）
- **手柄**：左摇杆按下 复制，右摇杆按下 追加复制
- **Android**：双指向左滑动 复制，双指向右滑动 追加复制
- **iOS**：双指向左滑动 复制，双指向右滑动 追加复制
- **Web**：双指向左滑动 复制，双指向右滑动 追加复制

所有无障碍手势均只向共享输入队列提交语义动作，再由引擎统一处理；平台桥接层不保存可复制文本、不伪造键盘事件，也不直接驱动游戏状态。这样键盘、手柄、触摸与系统辅助功能会遵循同一输入去重和复制逻辑。各平台既有的音效文件、桌面 WAV 的 2 倍增益，以及触觉时长保持不变。

Linux 无障碍版本需要安装 speech-dispatcher（提供 `spd-say`）：Debian/Ubuntu 可执行 `sudo apt install speech-dispatcher`，Arch 可执行 `sudo pacman -S speech-dispatcher`。
